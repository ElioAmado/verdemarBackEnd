...
class BookingControllerTest {
    ...
    void testUpdateBooking() throws Exception {
        ...
        mockMvc.perform(put("/api/bookings/4")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void testCheckPrice() throws Exception {
        Mockito.when(bookingService.getTotalPrice(eq((short) 1),
    ...
    void testGetDates() throws Exception {
        BookingDateRange range = new BookingDateRange(LocalDate.parse("2025-10-01"), LocalDate.parse("2025-10-10"));

        Mockito.when(bookingService.getAllDatesByApartment((short) 2)).thenReturn(List.of(range));

        mockMvc.perform(get("/api/bookings/getDates/2"))
                .andExpect(status().isOk())
