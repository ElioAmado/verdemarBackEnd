...
class PriceControllerTest {
    ...
    void testGetAllPricesDto() throws Exception {
        PriceResponseDTO dto = new PriceResponseDTO((short) 2, LocalDate.of(2025, 2, 1), new BigDecimal("200.00"));

        Mockito.when(priceService.getAllPriceDto()).thenReturn(List.of(dto));

        mockMvc.perform(get("/api/prices/dto"))
    ...
    void testGetPriceById() throws Exception {
        ...
        Mockito.when(priceService.getPriceById((short) 3, LocalDate.of(2025, 3, 1)))
                .thenReturn(price);

        mockMvc.perform(get("/api/prices/3/2025-03-01"))
                .andExpect(status().isOk())
    ...
    void testUpdatePrice() throws Exception {
        ...
        Mockito.when(priceService.updatePrice(eq((short) 4), eq(LocalDate.of(2025, 4, 1)), any(PriceRequestDTO.class)))
                .thenReturn(updatedPrice);

        mockMvc.perform(put("/api/prices/4/2025-04-01")
